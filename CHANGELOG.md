...
### Added
- **右上角 3 按钮**(替换 v1.2.x 单个 "—")⭐
  - 🗕 **缩小** — 最小化到系统托盘(原行为)
  - 🗖 **扩大** — 一键切换最大化 ↔ 还原(图标自动切 ☐ / ❐)
  - ✕ **关闭** — 隐藏到托盘(hover 红色提示,但不退出 App,按 `Ctrl + Alt + N` 唤回)
- **Todo 条目右键加"复制文本"** ⭐
  - 右键任意 todo 条目 → 菜单出现 **复制文本 / 删除** 两项
  - 点"复制文本" → todo 文字进剪贴板,可粘贴到任何地方
  - 删除功能保留

### UX 决议
- 关闭按钮 = 隐藏到托盘(macOS-style,sticky note 设计哲学:不"用完关闭",是"永远在背后")
- 真退出仍可通过托盘图标右键 → "退出程序"

### Internal
- 输入框 / slogan 输入 标准 QLineEdit 右键菜单(撤销/剪切/复制/粘贴/全选)自带,无需额外代码
